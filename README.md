...
```
market-insight-ai/
├── app/                  # código-fonte (pacote Python)
│   ├── main.py           # API FastAPI (entry point: main:app)
│   ├── analysis.py       # motor de análise e estratégias
│   ├── iq_service.py     # conexão IQ Option (iqair)
│   ├── news_service.py   # calendário econômico (Biquote)
│   ├── trade_manager.py  # gestão de entradas (SQLite)
│   ├── ai_advisor.py     # segunda opinião via LLM (opcional)
│   └── static/           # frontend (index.html, app.js, style.css)
├── tests/                # testes unitários (unittest)
├── run.py                # bootstrap: .env + dependências + uvicorn
├── setup.sh / setup.bat  # instalação (venv + requirements)
├── requirements.txt
...
Os indicadores vivem em `app/analysis.py`, no registro `INDICADORES_PADRAO`.
